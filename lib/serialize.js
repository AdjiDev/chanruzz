...
exports.smsg = (sock, m, store) => {
    ...
    if (m.message) {
        ...
        if (m.quoted) {
            ...
            m.quoted.copyNForward = (jid = m.chat, forceForward = false, options = {}) => sock.copyNForward(jid, vM, forceForward, options);
            m.quoted.download = () => sock.downloadMediaMessage(m.quoted);

            // New methods for quoted message reply, copy, and forward
            /**
    ...
    if (m.msg && m.msg.url) m.download = () => sock.downloadMediaMessage(m.msg);
